...
def mock_reporter(mocker):
    ...
    """
    mock_reporter_class = mocker.patch("greenkube.cli.report.ConsoleReporter")
...
def test_report_success(mocker, mock_reporter, sample_combined_metrics):
    ...
    """
    mock_repo = mocker.patch("greenkube.cli.report.get_combined_metrics_repository")
    repo_inst = MagicMock()
...
def test_report_namespace_not_found(mocker, mock_reporter, sample_combined_metrics):
    ...
    assert result.exit_code == 0
    mock_reporter.report.assert_called_once_with(data=[])
