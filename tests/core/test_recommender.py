...
def mock_combined_metrics():
    ...
    metric_zombie = CombinedMetric(
        timestamp=timestamp, pod_name="zombie-pod", namespace="default",
        total_cost=0.5, joules=10, cpu_request=500, memory_request=1073741824
    )
    
    # 2. Underutilized pod (CPU rightsizing)
    metric_rightsizing = CombinedMetric(
        timestamp=timestamp, pod_name="oversized-pod", namespace="prod",
        total_cost=1.2, joules=50000, cpu_request=1000, memory_request=2147483648,
    ...
    metric_healthy = CombinedMetric(
        timestamp=timestamp, pod_name="healthy-pod", namespace="prod",
        total_cost=1.0, joules=800000, cpu_request=1000, memory_request=2147483648
    )

    # 4. Pod with no "request" (cannot be rightsized)
    metric_no_request = CombinedMetric(
        timestamp=timestamp, pod_name="no-request-pod", namespace="dev",
        total_cost=0.1, joules=5000, cpu_request=0, memory_request=0
    )
