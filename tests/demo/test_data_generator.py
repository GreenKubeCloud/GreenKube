...
from greenkube.models.metrics import CombinedMetric, RecommendationRecord, RecommendationType
...
class TestGenerateNodeSnapshots:
    ...
    def test_node_names_match_topology(self):
        """Generated node names come from the demo topology."""
...
class TestGenerateCarbonIntensityHistory:
    ...
    def test_zone_matches_demo(self):
        """All records use the demo zone."""
        records = generate_carbon_intensity_history(days=1)
        for record in records:
            assert record["zone"] == DEMO_ZONE
    ...
    def test_intensity_is_realistic(self):
        """Carbon intensity should be in a realistic range (10-200 gCO2/kWh for France)."""
        records = generate_carbon_intensity_history(days=7)
        for record in records:
            assert 5 < record["carbonIntensity"] < 200
