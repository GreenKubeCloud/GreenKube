...
SAMPLE_HISTORY_DATA = [
    {
        ...
        'updatedAt': BASE_TIME.isoformat(), 'createdAt': BASE_TIME.isoformat(),
        'emissionFactorType': 'lifecycle', 'isEstimated': False, 'estimationMethod': None
    },
    {
    ...
    },
    {
        ...
        'updatedAt': BASE_TIME.isoformat(), 'createdAt': BASE_TIME.isoformat(),
        'emissionFactorType': 'lifecycle', 'isEstimated': True, 'estimationMethod': 'forecast'
    }
]
...
def test_save_history_new_records(sqlite_repo, db_connection):
    ...
    cursor.execute("SELECT carbon_intensity FROM carbon_intensity_history WHERE datetime=?", (SAMPLE_HISTORY_DATA[1]['datetime'],))
...
@pytest.mark.parametrize("query_time_str, expected_intensity", [
    # Exact match
    (BASE_TIME.isoformat(), 60.0), # 10:00 -> 10:00 record
    # Time between records
    ((BASE_TIME - timedelta(minutes=30)).isoformat(), 55.5), # 09:30 -> 09:00 record
